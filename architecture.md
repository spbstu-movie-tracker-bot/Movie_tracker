...
### 2.1. Сервер (Application Server)
Роль: Центральный компонент системы.
Функционал:
* Обрабатывает все входящие запросы от Telegram-бота.
* Координирует взаимодействие между Telegram-ботом, функциональными модулями и внешними сервисами.
* Обеспечивает маршрутизацию команд к соответствующим модулям.
...
### 2.4. Маршрутизатор команд (CommandRouter Module)
...
### 2.5. Модуль поиска фильмов (MovieSearch Module)
...
### 2.6. Модуль управления списком отслеживания (Watchlist Module)
...
### 2.7. Модуль рекомендаций (Recommendation Module)
...
### 2.8. Модуль авторизации (UserAuth Module)
...
### Таблица Users
Хранит информацию о зарегистрированных пользователях бота.
...
### Таблица Movies
Хранит метаданные фильмов, добавленных в списки отслеживания.

| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGSERIAL | Первичный ключ, автоинкремент |
| tmdb_id | INTEGER | Идентификатор фильма в TMDb (уникальный) |
| title | VARCHAR(500) | Название фильма |
| original_title | VARCHAR(500) | Оригинальное название |
| overview | TEXT | Описание сюжета |
| release_date | DATE | Дата выхода |
| poster_path | VARCHAR(500) | Путь к изображению постера |
| vote_average | DECIMAL(3,1) | Средний рейтинг |
| genres | VARCHAR(500) | Жанры фильма |
| popularity | DECIMAL(10,2) | Показатель популярности |

### Таблица Watchlist
Связующая таблица «многие ко многим» между пользователями и фильмами.

| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGSERIAL | Первичный ключ, автоинкремент |
| user_id | BIGINT | Внешний ключ → Users |
| movie_id | BIGINT | Внешний ключ → Movies |
| added_at | TIMESTAMP | Дата и время добавления фильма |
